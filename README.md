...
| `mos-geo-google-preferred-sources/` | Adds Google's "Add as a preferred source" button to a site: checks eligibility, detects the stack and brand, builds a button in the site's own style from Google's official code, places it after the best paragraph of each article, and verifies the live install with a script. Installs it directly when the site's code or WordPress is reachable, otherwise writes a developer handover and preview. Google's claim, and nothing more: readers who choose the site see it more in Top Stories and get a "preferred" badge in AI Overviews and AI Mode. Evidence: `mos-geo-google-preferred-sources/references/google-spec.md`. |
