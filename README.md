...
| `mos-geo-query-fan-out/` | For 1 to 500+ pages, finds the searches AI engines actually run (query fan-out) for the prompts each page should win, measures whether the page is cited today, and produces a per-page fix list to raise citability. Runs on DataForSEO LLM Responses and LLM Mentions, with a hard spend cap and a `retest` step to measure whether a fix actually raised citations. Sold as a hypothesis for citation lift, not a proven cause (`mos-geo-query-fan-out/references/evidence.md`). |
