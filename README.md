...
| `mos-geo-schema-scraper/` | Pulls the live JSON-LD from a list of URLs through an Apify real-browser actor, so it sees schema added by a tag manager or JavaScript, and saves one pure-JSON file per page (`[]` when a page has none) sorted by page type, plus the full run archive. The "before" snapshot for a schema rebuild, or for a competitor's pages. About $0.006 a URL, with a free dry run that shows every file it will write. |
